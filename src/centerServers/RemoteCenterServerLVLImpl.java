...
public class RemoteCenterServerLVLImpl implements RemoteCenterServerInterface
{
	...
	// Constructor
	public RemoteCenterServerLVLImpl()
	{
		if (!isInitialized)
		{
			...
			String managerId)
	{
		...
		if (id != null)
		{
			...
			logger.logToFile(cityAbbr + "[RecordManagerImpl.createTRecord()]: Success! createTRecord is done (ID: " + id + ")"
					+ " {CallerManagerID: " + managerId + "}");
			return "Success! createTRecord is done (ID: " + id + ")";
		}
	...
	public String createSRecord(String firstName, String lastName, String coursesRegistred, boolean status, String statusDate, String managerId)
	{
		...
		{
			logger.logToFile(cityAbbr + "[RecordManagerImpl.createSRecord()]: Error! createSRecord failed (at least one property was NULL)"
					+ " {CallerManagerID: " + managerId + "}");
		...
		if (id != null)
		{
			...
			logger.logToFile(cityAbbr + "[RecordManagerImpl.createSRecord()]: Success! createSRecord is done (ID: " + id + ")"
					+ " {CallerManagerID: " + managerId + "}");
			return "Success! createSRecord is done (ID: " + id + ")";
		}
	...
	public String getRecordsCount(String managerId)
	{
		...
		{
			(new RemoteCenterServerMTLImplService()).getRemoteCenterServerMTLImpl().recordExist("TR00001", "MTL0001");
			(new RemoteCenterServerDDOImplService()).getRemoteCenterServerDDOImpl().recordExist("TR00001", "DDO0001");
		...
		logger.logToFile(cityAbbr + "[RecordManagerImpl.getRecordCounts()]: Success! getRecordCounts is done" + " {CallerManagerID: " +
				managerId + "}");
		return result;
	}
	...
	public String editRecord(String recordId, String fieldName, String newValue, String managerId)
	{
		if ((recordId == null) || (fieldName == null) || (newValue == null) || (managerId == null))
		{
			logger.logToFile(cityAbbr +
					"[RecordManagerImpl.editRecord()]: Error! editRecord failed (recordId and/or fieldName and/or newValue is(are) NULL)"
					+ " {CallerManagerID: " + managerId + "}");
		...
		{
			logger.logToFile(cityAbbr +
					"[RecordManagerImpl.editRecord()]: Error! editRecord failed (recordId format is incorrect)" + " {CallerManagerID: "
					+ managerId + "}");
		...
		{
			logger.logToFile(cityAbbr +
					"[RecordManagerImpl.editRecord()]: Error! editRecord failed (There is no record with this ID)" + " {CallerManagerID: "
					+ managerId + "}");
		...
		if (ch.equals('T'))
		{
			synchronized (recordsMap)
			{
				synchronized (indexPerId)
				{
					...
					if (teacher == null)
					{
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Error! editRecord failed (Given ID doesn't exist)" +
								" {CallerManagerID: " + managerId + "}");
					...
					case "address":
						...
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: address" +
					...
					case "phoneNumber":
						...
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: phoneNumber" +
								" oldValue: " + oldValue + " newValue: " + newValue + " {CallerManagerID: " + managerId + "}");
						break;
					case "location":
						oldValue = teacher.getLocation();
						teacher.setLocation(newValue);
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: location" +
								" oldValue: " + oldValue + " newValue: " + newValue + " {CallerManagerID: " + managerId + "}");
						break;
					default:
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Error! editRecord failed (Given fieldName is invalid)" +
								" {CallerManagerID: " + managerId + "}");
			...
			synchronized (recordsMap)
			{
				synchronized (indexPerId)
				{
					...
					case "coursesRegistred":
						...
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: coursesRegistred"
								+ " oldValue: " + oldValue + " newValue: " + newValue + " {CallerManagerID: " + managerId + "}");
						break;
					case "status":
						boolean status;
						...
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: status" +
								" oldValue: " + oldValue + " newValue: " + newValue + " {CallerManagerID: " + managerId + "}");
						break;
					case "statusDate":
						Date date = new Date();
						...
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Success! editRecord is done for: statusDate" +
								" oldValue: " + oldValue + " newValue: " + newValue + " {CallerManagerID: " + managerId + "}");
						break;
					default:
						logger.logToFile(cityAbbr + "[RecordManagerImpl.editRecord()]: Error! editRecord failed (Given fieldName is invalid)" +
		...
		return "Success! Edit Record is done (Field: " + fieldName + ", New Value: " + newValue + ")";
	}
	...
	public boolean recordExist(String recordId, String managerId)
	{
		...
		{
			logger.logToFile(cityAbbr + "[RecordManagerImpl.recordExist()]: Error! editRecord failed (recordId format is incorrect)" +
					" {CallerManagerID: " + managerId + "}");
			return false;
		}
	...
	public String transferRecord(String managerId, String recordId, String remoteCenterServerName)
	{
		...
		if (recordId.toUpperCase().trim().charAt(0) == 'T') // Teacher record
		{
			...
			synchronized (recordsMap)
			{
				synchronized (indexPerId)
				{
					...
					if (teacher == null)
					{
						logger.logToFile(cityAbbr + "[RecordManagerImpl.transferRecord()]: Error! The given record dosen't exist in this server"
								+ " {CallerManagerID: " + managerId + "}");
						return "Error! The given record dosen't exist in this server";
					}
			...
			{
				RemoteCenterServerMTLImplService caller = new RemoteCenterServerMTLImplService();
				String result = caller.getRemoteCenterServerMTLImpl().createTRecord(teacher.getFirstName(), teacher.getLastName(), teacher.getAddress(),
						teacher.getPhone(), spec, remoteCenterServerName.toUpperCase(), remoteCenterServerName.toUpperCase() + "0001");

				logger.logToFile(cityAbbr + "[RecordManagerImpl.transferRecord()]: Success! Record Transfered {CallerManagerID: " + managerId + "}");
				return "Success! Record Transfered (" + result + ")";
			}
			if (remoteCenterServerName.toUpperCase().equals("LVL"))
			{
				RemoteCenterServerLVLImplService caller = new RemoteCenterServerLVLImplService();
				String result = caller.getRemoteCenterServerLVLImpl().createTRecord(teacher.getFirstName(), teacher.getLastName(), teacher.getAddress(),
						teacher.getPhone(), spec, remoteCenterServerName.toUpperCase(), remoteCenterServerName.toUpperCase() + "0001");
			...
			{
				RemoteCenterServerDDOImplService caller = new RemoteCenterServerDDOImplService();
				String result = caller.getRemoteCenterServerDDOImpl().createTRecord(teacher.getFirstName(), teacher.getLastName(), teacher.getAddress(),
						teacher.getPhone(), spec, remoteCenterServerName.toUpperCase(), remoteCenterServerName.toUpperCase() + "0001");
		...
		if (recordId.toUpperCase().trim().charAt(0) == 'S') // Student record
		{
			...
			synchronized (recordsMap)
			{
				synchronized (indexPerId)
				{
					...
					if (student == null)
					{
						logger.logToFile(cityAbbr + "[RecordManagerImpl.transferRecord()]: Error! The given record dosen't exist in this server"
								+ " {CallerManagerID: " + managerId + "}");
						return "Error! The given record dosen't exist in this server"; // The given record dosen't exist in this server
					}
			...
			{
				RemoteCenterServerMTLImplService caller = new RemoteCenterServerMTLImplService();
				String result = caller.getRemoteCenterServerMTLImpl().createSRecord(student.getFirstName(), student.getLastName(), courses,
			...
			{
				RemoteCenterServerLVLImplService caller = new RemoteCenterServerLVLImplService();
				String result = caller.getRemoteCenterServerLVLImpl().createSRecord(student.getFirstName(), student.getLastName(), courses,
						student.getStatus(), student.getStatusDate().toString(), remoteCenterServerName + "0001");
				logger.logToFile(cityAbbr + "[RecordManagerImpl.transferRecord()]: Success! Record Transfered {CallerManagerID: " + managerId + "}");
				return "Success! Record Transfered (" + result + ")";
			}
			...
			{
				RemoteCenterServerDDOImplService caller = new RemoteCenterServerDDOImplService();
				String result = caller.getRemoteCenterServerDDOImpl().createSRecord(student.getFirstName(), student.getLastName(), courses,
						student.getStatus(), student.getStatusDate().toString(), remoteCenterServerName + "0001");
				logger.logToFile(cityAbbr + "[RecordManagerImpl.transferRecord()]: Success! Record Transfered {CallerManagerID: " + managerId + "}");
				return "Success! Record Transfered (" + result + ")";
			}
	...
	private String produceNewId(String prefix, String managerId)
	{
		if (prefix.toUpperCase().equals("TR"))
		{
			...
			{
				logger.logToFile(cityAbbr + "[RecordManagerClass.produceNewId()]: Error! produceNewId failed (Teachers record number reached 99999)"
						+ " {CallerManagerID: " + managerId + "}");
				return null;
			}
		...
		if (prefix.toUpperCase().equals("SR"))
		{
			...
			{
				logger.logToFile(cityAbbr + "[RecordManagerClass.produceNewId()]: Error! produceNewId failed (Students record number reached 99999)"
						+ " {CallerManagerID: " + managerId + "}");
				return null;
			}
